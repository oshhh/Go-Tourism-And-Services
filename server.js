...
tables = {
    ...
    'hotel': ['service_provider_id','location_id', 'wifi_facility','ac','stars'],
...
function createDatabase(onComplete) {
    ...
    runQuery(callback, 'create table if not exists hotel(service_provider_id char(8) primary key,location_id char(8),wifi_facility char(1),ac char(1),stars int,foreign key (service_provider_id) references service_provider(service_provider_id),foreign key(location_id) references location(location_id),check (service_provider_id like \"HOT%\"),check (wifi_facility in (\"Y\", \"N\")),check (ac in (\"Y\", \"N\")),check (stars >= 0 and stars <= 5));')
