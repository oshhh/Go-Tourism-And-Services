...
tables = {
    ...
    'service_provider': ['service_provider_id', 'name', 'password', 'domain'],
    ...
    'trip': ['trip_id', 'departure_date', 'arrival_date', 'city'],
...
function createDatabase() {
    ...
    runQuery('drop table if exists visited;')
    runQuery('drop table if exists wishlist;')
    runQuery('drop table if exists query;')
    runQuery('drop table if exists service_request;')
    runQuery('drop table if exists trip;')
    runQuery('drop table if exists guide;')
    runQuery('drop table if exists tourist_spot;')
    runQuery('drop table if exists route;')
    runQuery('drop table if exists bus;')
    runQuery('drop table if exists taxi;')
    runQuery('drop table if exists flight;')
    runQuery('drop table if exists food_Item;')
    runQuery('drop table if exists food_item;')
    runQuery('drop table if exists restaurant;')
    runQuery('drop table if exists room;')
    runQuery('drop table if exists hotel;')
    runQuery('drop table if exists service;')
    runQuery('drop table if exists service_provider;')
    runQuery('drop table if exists administrator;')
    runQuery('drop table if exists user;')
    runQuery('drop table if exists neighbouring;')
    runQuery('drop table if exists location;')

    runQuery('create table if not exists location (location_id char(8) primary key,locality varchar(100) not null,city varchar(100) not null,state varchar(100) not null,country varchar(100) not null,pincode int,check (location_id like \"LOC%\"));')
    runQuery('create table if not exists neighbouring(location1_id char(8),location2_id char(8),distance_in_km int,primary key(location1_id, location2_id),foreign key (location1_id) references location(location_id),foreign key (location2_id) references location(location_id));')
    runQuery('create table if not exists user(user_id char(8) primary key,name varchar(100) not null,email varchar(100) not null,password varchar(100) not null,address varchar(100),phone_no char(10),location_id char(8),active char(1),foreign key(location_id) references location(location_id),check (user_id like \"USR%\"),check(active in (\"Y\", \"N\")));')
    runQuery('create table if not exists administrator(admin_id char(8) primary key,name varchar(100) not null, role varchar(100) not null,email varchar(100) not null,password varchar(100),check(admin_id like \"ADM%\"));')
    runQuery('create table if not exists service_provider(service_provider_id char(8) primary key,name varchar(100),password varchar(20),domain varchar(20),err1 char(1),check ((service_provider_id like \"HOT%\" and domain like \"hotel\") or    (service_provider_id like \"RES%\" and domain like \"restaurant\") or(service_provider_id like \"AIR%\" and domain like \"airline\") or (service_provider_id like \"TAP%\" and domain like \"taxi provider\") or    (service_provider_id like \"BPR%\" and domain like \"bus provider\") or (service_provider_id like \"TRP%\" and domain like \"train provider\") or   (service_provider_id like \"GUP%\" and domain like \"guide provider\")),check(err1 in (\"Y\", \"N\")) );')
    runQuery('create table if not exists service(service_id char(8) primary key,service_provider_id char(8),price float,discount int,foreign key (service_provider_id) references service_provider(service_provider_id),check (discount >= 0 and discount <= 100),check ((service_provider_id like \"HOT%\" and service_id like \"ROO%\") or  (service_provider_id like \"RES%\" and service_id like \"FOO%\") or(service_provider_id like \"AIR%\" and service_id like \"FLI%\") or  (service_provider_id like \"TAP%\" and service_id like \"TAX%\") or (service_provider_id like \"BPR%\" and service_id like \"BUS%\") or (service_provider_id like \"TRP%\" and service_id like \"TRA%\") or (service_provider_id like \"GUP%\" and service_id like \"GUI%\") ));')
    runQuery('create table if not exists hotel(service_provider_id char(8) primary key,location_id char(8),wifi_facility char(1),foreign key (service_provider_id) references service_provider(service_provider_id),foreign key(location_id) references location(location_id),check (service_provider_id like \"HOT%\"),check (wifi_facility in (\"Y\", \"N\")));')
    runQuery('create table if not exists room(service_id char(8) primary key,room_type varchar(100),capacity int,check(service_id like \"ROO%\"),foreign key(service_id) references service(service_id),check (capacity >= 1));')
    runQuery('create table if not exists restaurant(service_provider_id char(8) primary key,location_id char(8),delivers char(1),cuisine varchar(100),foreign key (service_provider_id) references service_provider(service_provider_id),check (service_provider_id like \"RES%\"),foreign key(location_id) references location(location_id),check (delivers in (\"Y\", \"N\")));')
    runQuery('create table if not exists food_item(service_id char(8) primary key,name varchar(100),cuisine varchar(100),foreign key(service_id) references service(service_id),check (service_id like \"FOO%\"));')
    runQuery('create table if not exists flight(service_id char(8) primary key,from_city varchar(100),to_city varchar(100),departure_time datetime,arrival_time datetime,foreign key(service_id) references service(service_id),check (service_id like \"FLI%\"),check (departure_time < arrival_time));')
    runQuery('create table if not exists taxi(service_id char(8) primary key,car_name varchar(100),capacity int,AC char(1),foreign key(service_id) references service(service_id),check (service_id like \"TAX%\"),check (capacity >= 0),check (AC in (\"Y\", \"N\")));')
    runQuery('create table if not exists bus(service_id char(8) primary key,from_location_id char(8),to_location_id char(8),active_days char(7),AC char(1),foreign key(service_id) references service(service_id),check (service_id like \"BUS%\"),foreign key(from_location_id) references location(location_id),foreign key(to_location_id) references location(location_id),check (active_days like \"[YN][YN][YN][YN][YN][YN][YN]\"),check (AC in (\"Y\", \"N\")));')
    runQuery('create table if not exists route (service_id char(8),location_id char(8),arrival_time time,primary key (service_id, location_id),foreign key(service_id) references service(service_id),foreign key(location_id) references location(location_id));')
    runQuery('create table if not exists tourist_spot (tourist_spot_id char(8) primary key,name varchar(100),location_id char(8),type varchar(100),entry_fee float,foreign key(location_id) references location(location_id));')
    runQuery('create table if not exists guide (service_id char(8) primary key,tourist_spot_id char(8),foreign key(service_id) references service(service_id),check (service_id like \"GUI%\"),foreign key(tourist_spot_id) references tourist_spot(tourist_spot_id));')
    runQuery('create table if not exists trip (trip_id char(8) primary key,departure_date date,arrival_date date,city varchar(100));')
    runQuery('create table if not exists service_request (request_id char(8) primary key,trip_id char(8) not null,service_id char(8) not null,timestamp datetime,quantity int not null,cost int not null,status varchar(15) not null,user_rating int,service_rating int,comments varchar(1000),foreign key(trip_id) references trip(trip_id),foreign key(service_id) references service(service_id),check (status in (\"Pending\", \"Accepted\", \"Rejected\", \"Completed\", \"Paid\")),check (user_rating >= 0 and user_rating <= 5),check (service_rating >= 0 and service_rating <= 5));')
    runQuery('create table if not exists query (query_id char(8), user_id char(8), query varchar(100),foreign key(user_id) references user(user_id));')
    runQuery('create table if not exists wishlist (user_id char(8), tourist_spot_id char(8), primary key(user_id, tourist_spot_id), foreign key(user_id) references user(user_id), foreign key(tourist_spot_id) references tourist_spot(tourist_spot_id))');
    runQuery('create table if not exists visited (trip_id char(8), tourist_spot_id char(8), primary key(trip_id, tourist_spot_id), foreign key(trip_id) references trip(trip_id), foreign key(tourist_spot_id) references tourist_spot(tourist_spot_id))');

\'Y\'),(\'HOT00037\',\'johmy\',\'johmy55\',\'Hotel\',\'Y\'),(\'HOT00038\',\'dopy\',\'dopy55\',\'Hotel\',\'Y\'),(\'HOT00039\',\'vog\',\'vog55\',\'Hotel\',\'Y\'),(\'HOT00040\',\'vol\',\'vol55\',\'Hotel\',\'Y\'),(\'HOT00041\',\'val\',\'val55\',\'Hotel\',\'Y\'),(\'HOT00042\',\'callyy\',\'callyy55\',\'Hotel\',\'Y\'),(\'HOT00044\',\'gooooooopy\',\'gooooooopy55\',\'Hotel\',\'Y\'),(\'HOT00045\',\'aqua\',\'aqua55\',\'Hotel\',\'Y\');')
...
async function runQuery(query, result) {
    var result;
    console.log(query);
    result = await con.query(query, function (err, result) {
            if (err) throw err;
            console.log(result);
            return result;
    });
...
function login_service_provider(user_id) {
    query = 'select password from service_provider where user_id = ' + user_id + ';'
    return runQuery(query);
}
function login_administrator(user_id) {
    query = 'select password from administrator where user_id = ' + user_id + ';'
    return runQuery(query);
}

// Deactivate or Remove
function deactivate_user(user) {
    runQuery('update user set active = \'N\' where user_id = ' + user_id + ';');
}
function deactivate_service_provider(service_provider_id) {
    runQuery('update service_provider set active = \'N\' where user_id = ' + user_id + ';');
}
function remove_administrator(service_provider_id) {
    runQuery('delete from service_provider where service_provider_id = ' + service_provider_id + ';');
}

// get locations matching the search
function getLocations(attribute_values) {
    runQuery('select * from location where ' + whereClause(attribute_values) + ';');
}
...
function getGeneralServiceProviderAndService(attribute_values, rating = 0) {
...
function getParticularServiceProviderAndService(service_provider, service, attribute_values, rating = 0) {
...
function getBusTrain(location1_id, location2_id, attribute_values) {
    attribute_values = inputAttributes(['bus', 'train'])
...
async function getFlight(from_location_id, to_location_id) {
...
function getTouristSpots(user_id, attribute_values, city, unvisited) {
    attribute_values = inputAttributes(['tourist_spot'])
    if(unvisited) {
        query = 'select * from tourist_spot, location where (tourist_spot_id not in (select tourist_spot_id from visited where user_id in ( select user_id from trip where trip_id = ' + trip_id +')) tourist_spot.location_id = location.location_id and location.city = ' + city + ' ) and (' + whereClause(attribute_values) + ');';
    } else {
        query = 'select * from tourist_spot, location where (tourist_spot.location_id = location.location_id and location.city = ' + city + ' ) and (' + whereClause(attribute_values) + ');';
    }
    runQuery(query);
}
...
function markTouristSpotVisited(trip_id, tourist_spot_id) {
    ...
    runQuery('delete from wishlist where user_id in (select user_id from trip where trip_id = ' + trip_id + ') and tourist_spot_id = ' + tourist_spot_id + ');')
}

// Filter trips (based on user_id and other attributes)
function getTrips(attribute_values) {
    runQuery('select * from trip where ' + whereClause(attribute_values) + ';');
}
...
function changeStatusOfServiceRequest(request_id, status) {
    runQuery('update service_request set status = ' + status + ' where request_id = ' + request_id + ';');
}
...
function getServiceRequests(user_id, attribute_values) {
    attribute_values = inputAttributes(['service_request']);
    runQuery('select * from service_request where trip_id in (select trip_id from trip where user_id = ' + user_id +') and (' + whereClause(attribute_values) + ');');
}

async function main() {
    await connect();
    // createDatabase();
    // console.log(await getFlight('\'LOC00015\'', '\'LOC00029\''));
    getTouristSpots(null, null, '\'Delhi\'', false);
