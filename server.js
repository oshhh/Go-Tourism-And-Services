...
function getGeneralServiceProviderAndService() {
    ...
    query = 'select * ' + 'from service_provider, service where( (service_provider.active = \'Y\') and (service_provider.service_provider_id = service.service_provider_id) and (' + whereClause(attribute_values) +  '));'
    runQuery(query);
}

// Query on a particular service provider and service based on the attributes
function getParticularServiceProviderAndService(service_provider, service, attribute_values = null) {
...
// Get a list of tourist spots in your wishlist in a city 
...
function makeItinerary(user, destination) {
    trip = {}
    _insert_('trip', trip);
...
function createDatabase() {
    ...
    runQuery('create table if not exists user(user_id char(8) primary key,name varchar(100) not null,email varchar(100) not null,password varchar(100) not null,address varchar(100),phone_no char(10),location_id char(8),err char(1),foreign key(location_id) references location(location_id),check (user_id like \"USR%\"),check(err in (\"Y\", \"N\")));')
    runQuery('create table if not exists administrator(admin_id char(8) primary key,name varchar(100) not null, role varchar(100) not null,email varchar(100) not null,passwor_admin varchar(100),check(admin_id like \"ADM%\"));')
    runQuery('create table if not exists service_provider(service_provider_id char(8) primary key,name varchar(100),dom_pass varchar(20),domain varchar(20),err1 char(1),check ((service_provider_id like \"HOT%\" and domain like \"hotel\") or    (service_provider_id like \"RES%\" and domain like \"restaurant\") or(service_provider_id like \"AIR%\" and domain like \"airline\") or (service_provider_id like \"TAP%\" and domain like \"taxi provider\") or    (service_provider_id like \"BPR%\" and domain like \"bus provider\") or (service_provider_id like \"TRP%\" and domain like \"train provider\") or   (service_provider_id like \"GUP%\" and domain like \"guide provider\")),check(err1 in (\"Y\", \"N\")) );')
    ...
    runQuery('create table if not exists trip (trip_id char(8) primary key,departure_date date,arrival_date date,trip_city varchar(100);')
